...
function OrdersTab({ user, ar, t }: { user: any; ar: boolean; t: any }) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "cancelled": case "refunded": return <XCircle className="w-4 h-4 text-red-500" />;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "cancelled": case "refunded": return "bg-red-50 text-red-700 border border-red-200";
    ...
    return (
      <div className="space-y-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="animate-pulse bg-[#FCF8FF] rounded-2xl h-32 border border-[#E7D8F1]" />
        ))}
      </div>
    );
  }
          ...
          <div key={order.id} className="bg-white rounded-2xl p-5 border border-[#E7D8F1] shadow-sm hover:shadow-md transition-shadow">
            <div className="flex flex-wrap justify-between items-center gap-3 mb-4">
              <div>
                ...
                <p className="text-xs text-[#8D7A97] mt-0.5">
                  {new Date(order.createdAt).toLocaleDateString(ar ? "ar-EG" : "en-US", {
            ...
            <div className="border-t border-[#F2EAFA] pt-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="flex flex-wrap gap-x-8 gap-y-2">
                <div>
                  <p className="text-[10px] text-[#8D7A97] font-bold uppercase">{ar ? "الإجمالي" : "Total"}</p>
                  <p className="font-extrabold text-[#4B1C71]">{parseFloat(order.total).toFixed(0)} {t.currency}</p>
                </div>
                <div>
                  <p className="text-[10px] text-[#8D7A97] font-bold uppercase">{ar ? "طريقة الدفع" : "Payment"}</p>
                  <p className="text-sm font-semibold text-[#4B1C71]">{ar ? "الدفع عند الاستلام" : "COD"}</p>
                </div>
                <div>
                  <p className="text-[10px] text-[#8D7A97] font-bold uppercase">{ar ? "العنوان" : "Shipping To"}</p>
                  <p className="text-sm font-semibold text-[#4B1C71] truncate max-w-xs">{order.shippingAddress}</p>
                </div>
              </div>
